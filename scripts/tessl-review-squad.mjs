...
function parseSkill(filePath) {
  ...
  const author =
    getField(frontmatter, "author") ?? getMetadataField(frontmatter, "author");
  const createdAt =
...
const selectedFiles =
  requested.length > 0
    ? requested.map((file) => path.resolve(repoRoot, file))
    : fs
        .readdirSync(skillsDir)
        .filter((name) => name.startsWith("squad-") && name.endsWith(".md"))
        .sort()
