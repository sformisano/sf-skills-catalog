...
3. Record that path in `manifest.yaml` as `current.heartbeat_path`.
...
1. Record dispatch notes with role, purpose, output path, heartbeat path when used, phase, round, critical-path status, and dispatch timestamp.
...
5. On child completion, inspect the returned artifact or blocker immediately, update the manifest, clear `current.heartbeat_path` when applicable, send the completion heartbeat, and trigger the next lifecycle step.
