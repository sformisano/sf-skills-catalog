...
description: "Defines the required shape of a squad lifecycle plan artifact. Use when loaded by squad plan author, squad plan critic, or squad lead to write or check a squad plan, not for ordinary coding plans."
metadata:
  skillcatalog/display_name: "Squad Plan Verification"
...
3. Add the claim, mechanism, and proof matrix for load-bearing claims.
4. Add verification inputs that downstream implementation and review can run without rediscovery.
5. Add any optional sections only when they improve execution quality.
6. Run the quality checks before handing the plan to critique or implementation.
...
See `references/squad-plan-walkthrough-archetypes.md` for the archetype templates.
...
### Claim/Mechanism/Proof Matrix

**Tier trigger:** required for Standard and Full tier plans. Lite plans may omit it only when all claims are trivial and directly covered by `## Verification Inputs`.

Use `references/squad-claim-proof-matrix.md` for the table contract and special claim rules.

The matrix is mandatory whenever the plan includes:

- any NNG
- any high-risk REQ, as defined in `references/squad-claim-proof-matrix.md`
- external behavior, compatibility, persistence, authority, shared-contract, security, safety, compliance, or regression scope
- claims that assert an operator-visible behavior change, runtime safety property, removed or renamed surface, fixture-backed behavior, generated artifact, persistence change, authority change, migration, or shared contract
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CLAIM-001 | NNG-01 | CLI contract remains stable. | Parser keeps the existing option and maps it to the new internal path. | CLI parser | CLI users, help output, integration tests | compatibility command with old option | SURFACE-001 | PHASE-01 |

Rules:

- every NNG gets at least one row
- every high-risk REQ gets at least one row
- `mechanism in code` must name a concrete mechanism, not intent
- `proof_stage` must use one allowed value from `references/squad-claim-proof-matrix.md`: `PHASE-XX`, `PHASE-ALL`, `cross_phase_smoke`, `e2e`, or `delivery`
- broad suite success may support a row, but cannot be the only proof for NNG or high-risk REQ claims
- negative claims need a negative proof or a surface inventory row over the real public or generated surfaces
- fixture-backed claims must point to a fixture inventory row, not just the fixture path
- later-stage proof deferrals are allowed only when `## Verification Inputs` names the later proof input and `## Phase Integrity` explains why earlier phases may close locally without final proof

### Fixture and Surface Inventory

Required when the claim matrix names a fixture, public surface, generated artifact, or removed surface.

Fixture rows:

| inventory_id | fixture | producer or setup | consuming test or exercise | assertion or expected signal |
| --- | --- | --- | --- | --- |
| FIXTURE-001 | `tests/fixtures/example.json` | checked into repo | `example_round_trips` | fails if the fixture is not parsed |

Surface rows:

| inventory_id | surface | files or commands | claim_ids | scan or exercise |
| --- | --- | --- | --- | --- |
| SURFACE-001 | CLI help | `tool --help` | CLAIM-001 | positive compatibility check plus negative wording scan |

Rules:

- every fixture listed by the plan must have a consuming test or exercise, and fixture-backed matrix rows should reference the `FIXTURE-XXX` row
- every removed field, option, UI control, docs phrase, or generated property must have a negative scan scope, and removed-surface matrix rows should reference the `SURFACE-XXX` row
- include UI copy, CLI help, docs, embedded catalogs, generated schemas, and system specs when those surfaces exist and the claim could appear there
...
| proof obligation | command or action | fixture or setup | expected signal | evidence location | does not prove |
| --- | --- | --- | --- | --- | --- |
| CLAIM-001 feature exercise | run the documented command or user flow | named fixture, seed data, or environment knob | the externally observable result named in the plan | implementation or review artifact section that will carry the fresh evidence | absence from generated docs, unless paired with a negative scan |

Rules:

- include one row for every non-trivial proof path the plan expects downstream agents to run
- when `## Claim/Mechanism/Proof Matrix` exists, every NNG or high-risk REQ claim row must have a corresponding direct proof input or an explicit rationale for why review can inspect it without a command
...
- `does not prove` names the important gap left by a broad-suite, positive-only, or fixture-setup proof. Use `N/A` when the proof has no meaningful residual gap.
...
- `## Claim/Mechanism/Proof Matrix` exists when required by tier, NNGs, high-risk REQs, scope flags, or trigger principles from `references/squad-claim-proof-matrix.md`
- every NNG or high-risk REQ claim row names a concrete code mechanism and direct proof; broad suite success alone is not enough
- every NNG or high-risk REQ claim row names an allowed `proof_stage`, and integrated-only claims use `e2e` rather than a premature phase owner
- every later-stage `proof_stage` row has a matching `## Verification Inputs` row and, for phased plans, `## Phase Integrity` explains the accepted proof deferral
- every fixture-backed claim has a consuming test or exercise in `## Fixture and Surface Inventory`
- every removed field, option, UI control, command, schema property, docs phrase, or embedded concept has a negative scan scope
